...
router.get("/products", async (req, res, next) => {
    try {
        ...
        return res.status(500).send({ message: "Error while getting products", err: err })
    }
})
...
router.get("/orders/:userContact", async (req, res, next) => {
    try {
        ...
        return res.status(500).send({ message: "Error while getting user orders", err: err })
    }
});
...
router.post("/product", upload.single("image"), async (req, res, next) => {
    try {
        ...
        return res.status(201).send({ message: 'Successfully added', data: result });
    } catch (err) {
        return res.status(500).send({ message: "Error while getting updating product", err: err })
    }
})

router.post("/product/update/:productId", upload.single("image"), async (req, res, next) => {
    try {
        const image = req.file;
        const {
            full_name,
        ...
        const result = await updateProductById(req.params.productId, {
            full_name,
            full_model,
            price_usd,
            percent_3m,
        ...
        return res.status(201).send({ message: 'Successfully updated', data: result });
    } catch (err) {
        return res.status(500).send({ message: "Error while getting updating product", err: err })
    }
})
...
router.get("/currency", async (req, res, next) => {
    try {
        ...
        if (!currencyRate) return res.status(404).send({ message: 'Currency rate not found' });
        return res.status(200).send({ message: 'Successfully fetched', data: currencyRate });
    } catch (err) {
        return res.status(500).send({ message: "Error while getting updating product", err: err })
    }
});
...
router.get("/currency/update", async (req, res, next) => {
    try {
        ...
        if (!result) return res.status(404).send({ message: 'Currency rate not updated' });
